...
//bonobo::mesh_data
//parametric_shapes::createQuad3(float const width, float const height,
...
//        };
//
//        float x = width / (horizontal_split_count + 1);
//        float z = height / (vertical_split_count + 1);
//
...
//                Vertex vertex;
//                vertex.position = glm::vec3(i * x - width * 0.5f, 0.f, j * z - height * 0.5f);
    ...
    unsigned int const vertical_split_count)
{
    ...
    for (unsigned int i = 0; i <= horizontal_split_count + 1; ++i) {
        for (unsigned int j = 0; j <= vertical_split_count + 1; ++j) {
            Vertex vertex;
            //vertex.position = glm::vec3(i * x - width * 0.5f, 0.f, j * z - height * 0.5f);
            vertex.position = glm::vec3(i * x - width * 0.5f, j * z - height * 0.5f, 0.f );
            vertex.texcoord = glm::vec2(
                static_cast<float>(i) / static_cast<float>(horizontal_split_count + 1),
                static_cast<float>(j) / static_cast<float>(vertical_split_count + 1));
            vertices.push_back(vertex);
...
bonobo::mesh_data
parametric_shapes::createQuad2(float const width, float const height,
                                ...
                                unsigned int const latitude_split_count)
{
    ...
    auto const longitude_edges_count = longitude_split_count + 2u;
    auto const latitude_edges_count = latitude_split_count + 2u;
    auto const vertices_nb = longitude_edges_count * latitude_edges_count;
    //auto const vertices_nb = longitude_split_count * latitude_split_count;
    ...
    float theta = 0.0f;
    float d_theata = glm::two_pi<float>() / (static_cast<float>(longitude_edges_count - 1u) );
    float phi = 0.0f;
    float d_phi = glm::pi<float>() / (static_cast<float>(latitude_edges_count - 1u) );
    ...
    for (unsigned int i = 0u; i < longitude_edges_count; ++i){
        ...
        for (unsigned int j = 0u; j < latitude_edges_count; ++j){
            ...
            texcoords[index] = glm::vec3(static_cast<float>(i) / (static_cast<float>(longitude_edges_count) ),
                                         static_cast<float>(j) / (static_cast<float>(latitude_edges_count) ), 0.0f);
            // tangent(simplified -> let phi=pi/2)
             ...
             */
            // simplified version -> let theta = 0
            auto b = glm::vec3(0.0f, radius * sin_phi, radius * cos_phi);
            b = glm::normalize(b);
            binormals[index] = b;
    ...
    // create index array
    auto index_sets = std::vector<glm::uvec3>(2u * (longitude_edges_count - 1u) * (latitude_edges_count - 1u));

    // generate indices iteratively
    index = 0u;
    for (unsigned int i = 0u; i < longitude_edges_count - 1u; ++i)
    {
        for (unsigned int j = 0u; j < latitude_edges_count - 1u; ++j)
        {

            index_sets[index] = glm::uvec3(latitude_edges_count * (i + 0u) + (j + 0u),
                                           latitude_edges_count * (i + 0u) + (j + 1u),
                                           latitude_edges_count * (i + 1u) + (j + 1u));
            ++index;

            index_sets[index] = glm::uvec3(latitude_edges_count * (i + 0u) + (j + 0u),
                                           latitude_edges_count * (i + 1u) + (j + 1u),
                                           latitude_edges_count * (i + 1u) + (j + 0u));
            ++index;
        }
    }
...
{
    //! \todo Implement this function
    auto const major_edges_count = major_split_count + 2u;
    auto const minor_edges_count = minor_split_count + 2u;
    auto const vertices_nb = major_edges_count * minor_edges_count;
    ...
    float theta = 0.0f;
    float d_theata = glm::two_pi<float>() / (static_cast<float>(major_edges_count - 1u) );
    float phi = 0.0f;
    float d_phi = glm::two_pi<float>() / (static_cast<float>(minor_edges_count - 1u) );
    ...
    for (unsigned int i = 0u; i < major_edges_count; ++i){
        ...
        for (unsigned int j = 0u; j < minor_edges_count; ++j){
            ...
            //texture coordinates
            texcoords[index] = glm::vec3(static_cast<float>(i) / (static_cast<float>(major_edges_count) ),
                                         static_cast<float>(j) / (static_cast<float>(minor_edges_count) ), 0.0f);
            // tangent(simplified -> let phi=pi/2)
            auto t = glm::vec3(-minor_radius * sin_theta * cos_phi,
            ...
            // unsimplified version
            auto b = glm::vec3(-(major_radius + minor_radius * cos_theta) * sin_phi,
    ...
    // create index array
    auto index_sets = std::vector<glm::uvec3>(2u * (major_edges_count - 1u) * (minor_edges_count - 1u));

    // generate indices iteratively
    index = 0u;
    for (unsigned int i = 0u; i < major_edges_count - 1u; ++i)
    {
        for (unsigned int j = 0u; j < minor_edges_count - 1u; ++j)
        {

            index_sets[index] = glm::uvec3(minor_edges_count * (i + 0u) + (j + 0u),
                                           minor_edges_count * (i + 0u) + (j + 1u),
                                           minor_edges_count * (i + 1u) + (j + 1u));
            ++index;

            index_sets[index] = glm::uvec3(minor_edges_count * (i + 0u) + (j + 0u),
                                           minor_edges_count * (i + 1u) + (j + 1u),
                                           minor_edges_count * (i + 1u) + (j + 0u));
            ++index;
        }
    }
...
bonobo::mesh_data
parametric_shapes::createQuad3(unsigned int width, unsigned int height)
{
