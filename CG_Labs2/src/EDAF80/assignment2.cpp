...
edaf80::Assignment2::run()
{
    ...
    // set mode: 1.circleRing 2.quad 3.sphere 4.torus
    auto const mode = 4u;
    auto const shape_circleRing = parametric_shapes::createCircleRing(2.0f, 0.75f, 40u, 4u);
    auto const shape_quad = parametric_shapes::createQuad(0.25f, 0.15f); //ex1
    auto const shape_sphere = parametric_shapes::createSphere(0.15f, 10u, 10u); //ex2
    auto const shape_torus = parametric_shapes::createTorus(0.15f, 0.05f, 50u, 50u); //ex3
    bonobo::mesh_data shape;
    if (mode == 1u){
        shape = shape_circleRing;
    }else if (mode == 2u){
        shape = shape_quad;
    }else if (mode == 3u){
        shape = shape_sphere;
    ...
    //auto const shape = shape_torus;
    if (shape.vao == 0u)
        return;

    // Set up the camera
    mCamera.mWorld.SetTranslate(glm::vec3(0.0f, 1.0f, 9.0f)); // ex2: (0,0,0.5)
    mCamera.mMouseSensitivity = glm::vec2(0.003f);
    mCamera.mMovementSpeed = glm::vec3(3.0f); // 3 m/s => 10.8 km/h
    // mCamera.mWorld.SetRotateX(glm::half_pi<float>()); // ex2: position the camera below the sphere
    ...
    auto model = Node();
    model.set_geometry(shape);
    model.set_program(&fallback_shader, set_uniforms);
    TRSTransformf& model_transform_ref = model.get_transform();
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Define initial value of the start position and velocity
        unsigned int num_points = control_point_locations.size();
        float path_pos = 0.0f;
        float velocity = elapsed_time_s;
        //model.get_transform().RotateY(0.01f);
        ...
        if (interpolate) {
            ...
            if (use_linear) {
                ...
                std::cout << elapsed_time_s << std::endl; // for debugging
                glm::vec3 newPosition = interpolation::evalLERP(control_point_locations[i%num_points], control_point_locations[(i+1)%num_points] , path_pos-i);
                glm::vec3 newPosition2 = interpolation::evalLERP(control_point_locations[i%num_points], control_point_locations[(i+1)%num_points] , path_pos-i+0.1);
                model.get_transform().SetTranslate(newPosition);
                model.get_transform().LookAt(newPosition2);
                //path_pos += velocity;
            }
            else {
                //! \todo Compute the interpolated position
                //!       using the +-Rom interpolation;
                //!       use the `catmull_rom_tension`
                //!       variable as your tension argument.
                path_pos += velocity;
                int i = floor(path_pos);
                glm::vec3 newPosition = interpolation::evalCatmullRom(control_point_locations[i%num_points], control_point_locations[(i+1)%num_points], control_point_locations[(i+2)%num_points], control_point_locations[(i+3)%num_points], catmull_rom_tension, path_pos - i);
                glm::vec3 newPosition2 = interpolation::evalCatmullRom(control_point_locations[i%num_points], control_point_locations[(i+1)%num_points], control_point_locations[(i+2)%num_points], control_point_locations[(i+3)%num_points], catmull_rom_tension, path_pos - i+0.1);
                model.get_transform().SetTranslate(newPosition);
                model.get_transform().LookAt(newPosition2); // let the shape face to the next point
            }
        }

        model.render(mCamera.GetWorldToClipMatrix());
        ...
        if (opened) {
            ...
            if (selection_result.was_selection_changed) {
                model.set_program(selection_result.program, set_uniforms);
            }
            ImGui::Separator();
