...
//    camera.mWorld.SetTranslate(glm::vec3(0.0f, 0.0f, 6.0f));
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        stack<CelestialBodyRef> sceneGraph;
        CelestialBodyRef sunref = {&sun,glm::mat4(1.0f)};
        sceneGraph.push(sunref);
        
        while (!sceneGraph.empty())
        {
            CelestialBodyRef curref = sceneGraph.top();
            sceneGraph.pop();
            
            glm::mat4 child_transform = curref.body->render(animation_delta_time_us, camera.GetWorldToClipMatrix(), curref.parent_transform,show_basis);
            
            for (auto child : curref.body->get_children()){
                CelestialBodyRef childref = {child, child_transform};
                sceneGraph.push(childref);
            }
...
//        moon.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), glm::mat4(1.0f), show_basis);
        //exercise4
//        glm::mat4 earth2world = earth.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), glm::translate(glm::mat4(1.0f), glm::vec3(1.0f, 0.0f, 0.0f)), show_basis);
//        moon.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), earth2world,show_basis);
