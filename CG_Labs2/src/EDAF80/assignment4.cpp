...
{
	// Set up the camera
	mCamera.mWorld.SetTranslate(glm::vec3(-40.0f, 14.0f, 6.0f)); //-40.0f, 14.0f, 6.0f
    ...
    GLuint normalmap_texture = bonobo::loadTexture2D(config::resources_path("textures/waves.png"));


	//
	// Todo: Load your geometry
	//
    auto const set_water_uniforms = [&light_position, &camera_position, &elapsed_time_s](GLuint program){
        glUniform3fv(glGetUniformLocation(program, "light_position"), 1, glm::value_ptr(light_position));
        glUniform3fv(glGetUniformLocation(program, "camera_position"), 1, glm::value_ptr(camera_position));
        glUniform1f(glGetUniformLocation(program, "time"), elapsed_time_s);
    };
    
    auto const shape_quad = parametric_shapes::createQuad(100.0f, 100.0f, 1000u, 1000u);
    ...
    quad.add_texture("skybox_texture", cubemap, GL_TEXTURE_CUBE_MAP);
    quad.add_texture("water_texture", normalmap_texture, GL_TEXTURE_2D);
    quad.set_program(&water_shader, set_water_uniforms);
